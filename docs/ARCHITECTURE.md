...
`AIPanel` 发送时按设置勾选拼 system prompt：

```
你是 markio 内嵌的写作助手 …
[ 当前打开的笔记内容，前 6000 字 ]            ← aiUseCurrentFile=true
[ 仓库内 grep 命中的片段，±3 行，最多 5 条 ]   ← aiUseWorkspace=true
```

`aiUseWorkspace` 走的是后端 `ai_retrieve(workspace, query, k=5)`：

- 内部直接复用 `fs_ops::grep()`（≤ 2 MB / 文件，≤ 3000 文件）
- 把命中行往上下各拓 3 行作 snippet
- 文件名命中时取文件前 1000 字作 snippet
- **关键词命中**而非语义相似，对同义改写 / 隐喻 / 长概念支持弱

这套是 **占位实现**，能让 AI"看到"仓库片段，但不是真 RAG。

### 演进路径：真向量 RAG

如果要让 AI 真的"基于目录分析"，标准做法是 embed + 向量检索：

1. **嵌入模型**
   - 本地优先：Ollama 跑 `nomic-embed-text` / `bge-large-zh`，0 成本、不出网
   - 云：OpenAI `text-embedding-3-small`（Anthropic 暂无 embedding endpoint）
2. **向量库（in-process）**
   - **首选 `sqlite-vec`**：单文件 `.markio/index.sqlite`，Rust 通过 `rusqlite` + `sqlite-vec` 扩展直接用，零运维
   - 备选纯 Rust：`hnsw_rs` / `instant-distance` 写到自定义文件；不需要 sqlite 但 query 接口要自己写
3. **索引流程**（后台 worker）
   - workspace 加载时启一个 Tokio task：walk → 跳过已索引（按 file mtime+hash 判）→ chunk（按 markdown 块边界，~400 字一段）→ batch 调 embedding API → 写 vec 表
   - 文件保存后增量重索引该文件
4. **查询时机**
   - `ai_chat` 前置一个 `ai_retrieve_semantic(workspace, query, k)`：embed query → vec_search → 取 top-k chunk → 拼 system prompt
   - 同时保留关键词 retrieve 作 fallback

### 落地清单（要做时按这个推）

| 步骤 | 改什么 | 工作量 |
| --- | --- | --- |
| 1 | `Cargo.toml` 加 `rusqlite` + `sqlite-vec` | 5 分 |
| 2 | 新 `src-tauri/src/index.rs`：schema、chunk 算法、增量更新 | 1 天 |
| 3 | `ai_chat` 前加 embed 调用：用 secrets 里的 key 调 OpenAI embeddings，或调本地 Ollama `/api/embeddings` | 0.5 天 |
| 4 | UI：仓库设置里加"重建索引"按钮 + 索引进度条；status bar 多一栏 `已索引 N / M 个文档` | 0.5 天 |
| 5 | 设置里多个开关 `aiSemanticRetrieve: boolean` 并跟 keyword retrieve 互斥 | 0.5 小时 |
| 6 | Mac App Store 沙盒下 sqlite 数据库路径要走 `tauri::path::AppLocalDataDir`；记得 entitlements 把它列上 | 1 小时 |

实测 macOS M2 + Ollama `nomic-embed-text` 跑 1000 个 markdown chunk ≈ 30 秒；查询 < 100 ms。这套是真能上的。

## 已知 trade-off

- **小仓库的文件名搜索** 仍在前端走（树是已扫好的，filter 是 JS）。> 1 万节点要 Rust 索引
- **当前文档查找** 是 React 在 preview DOM 上 walkNodes，>10万字会卡
- **AI 仓库分析** 当前是 keyword grep 而非向量；演进见上面「AI · 上下文检索」
- **真同步** 还是 UI 壳
