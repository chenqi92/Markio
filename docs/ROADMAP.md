...
| 文件树 / 快速打开 / 全文搜索 | 🟡 可用但需加固 | 文件树已有虚拟列表；全文搜索命中可定位到行并显示截断上限；文件树仍有深度、数量、隐藏文件、只显示 markdown 等硬限制 |
...
- Git 自动同步遇到 pull 冲突时，`local` / `remote` 策略会执行对应的 ours / theirs 解决、提交 merge 结果并继续 push；`ask` / `newest` 仍停在冲突状态等待人工确认。
- 崩溃摘要上报、RAG 状态订阅、文件监听订阅和文件变化后的目录刷新失败会进入后台诊断中心，不再只写 console 或静默吞错。
...
- `syncConflictStrategy` 继续补 Git `newest` 策略和云同步接入，不再只是设置项。
- 后台任务失败继续补导入、云存储和设置页长任务的诊断入口。
